...
export class ElementList{
    ...
    *get(index: number) {
        ...
        for (let i = 0; i < this.count; i++) yield this.element.children[index + i];
    }
    set(index: number, value: any) {
        index = index * this.count;
        if (index >= this.element.children.length) return false;
        if (!(value instanceof Node)) value = this.render(value);
    ...
    pop() {
        ...
        let el = this.element.children[index], el2: Element | null;
    ...
    shift() {
        const result: Element[] = [];
        let el: Element = this.element.firstElementChild, el2: Element | null;;
    ...
    splice(start: number, deleteCount?: number, ...items: any[]) {
        ...
        if (deleteCount === undefined) deleteCount = this.element.children.length - start;
    ...
    move(from: number, to: number) {
        ...
        let el: Element = this.element.children[from], el2: Element | null;
        for (let i = 0; i < this.count; i++) {
            if (!el) break;
            movers.push(el);
            el2 = el.nextElementSibling;
    ...
    *[Symbol.iterator]() {
        for (let element of this.element.children) yield element;
    }
}
