...
export interface IReactivityPrefix {
    /**
     * Prefix for attribute templates
     */
    attr?: string;
    /**
     * Prefix for property templates
     */
    prop?: string;
}

export interface IReactivityAttrs {
    render?: string;                  // if something (such as object) reprocess all element attrs & props and all nested reactivity directives.
    if?: string;                      // render if something is returned; remove if not.
    next?: string;                    // clone and render after if something is returned.
...
export interface IReactivityInit {
    context?: any;
    el?: Element;
    prefix?: IReactivityPrefix;
}

export interface IReactOptions {
    /**
     * comma separated string used to filter the other options to determine 
     * what should be rendered
     */
    to?: string;
    /**
     * Only these attributes will be considered for rendering if specified.
     * Specified as a comma-separated string. Use the empty string to consider 
     * no props.
     */
    attrs?: string;       // comma separated
    /**
     * Only these properties will be considered for rendering if specified.
     * Specified as a comma-separated string. Use the empty string to consider 
     * no attributes.
     */
    props?: string;       // comma separated
    /**
     * Only the templates created on these comments will be considered for 
     * rendering if specified Specified as an Array of nodes. Use an empty 
     * array to consider no comments.
     */
    comments?: Comment[]       // Array of comment nodes
}
...
export class Reactivity {
     ...
     * 
     */
    commentTemplates = new WeakMap<Node, [string, Node]>();
    comments: WeakRef<Comment>[] = [];

    /**
     * All compiled comment templates
     */
    compiledCommentTemplates = new WeakMap<Element, Function>();

    /**
     * Maps the reactivity attributes to the nams of the real node attributes.
     * Attributes self-contain their templates.
     */
    attrTargets = new WeakMap<Attr, string>();
    attrs: WeakRef<Attr>[] = [];

    /**
     * All compiled attribute templates
     */
    compiledAttrTemplates = new WeakMap<Attr, Function>();

    /**
     * The reactivity props to the names of node properties they set.
     */
    propTargets = new WeakMap<Attr, string>();
    props: WeakRef<Attr>[] = [];

    /**
     * All compiled property templates
     */
    compiledPropTemplates = new WeakMap<Attr, Function>();
     ...
     * @returns 
     */
    compileAttrTemplates(...attrs: Attr[]) {
     ...
     * @returns 
     */
    compilePropTemplates(...props: Attr[]) {
     ...
     * @returns 
     */
    compileCommentTemplates(...comments: Comment[]) {
