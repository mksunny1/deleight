...
export type IIndexToIndex = {[key: number]: number};
...
export class RefType {
            ...
            if (!this.elements || !(this.elements.includes(element))) {
                for (rAttr of [refAttr, iterAttr]) {
                    if (element.hasAttribute(rAttr)) {
                        ...
                        if (addIndex && reftype instanceof IterRefType) reftype.addIndex = true;
    ...
    react(...refs: string[] | [IStrObject]) {
        ...
        if (this.refs === null && !this.hidden) return this.hide();  // please note the return. proceeding can lead to errors when getting
        ...
        if (refs.length) {   // call a subset
            ...
            if (this.hidden && this.refs !== null) this.show();  // automatically show
    ...
    get(ref: string) {
        ...
        if ((ref === this.options?.self) || (ref === options.self)) return this.refs;
        const prop = this.destructure(ref);
        if (!prop) return;
...
export class IterRefType extends RefType {
    ...
    push(...items: any[]) {
        let result;
        if (this.refs.push) result = this.refs.push(...items);
        let element: Element;
    ...
    pop() {
        let result;
        if (this.refs.pop) result = this.refs.pop();
    ...
    splice(start: number, deleteCount?: number, ...items: any[]) {
        let result;
        if (this.refs.splice) result = this.refs.splice(start, deleteCount, ...items);
     ...
     * @param swap
     */
    move(i1: number, i2: number, swap?: boolean) {
        if (i1 === i2) return;   // nothing to do here.
        ...
        let sources: {number?: RefType[]}, anchors: {number?: (Node|Node[])[]}, 
        childElement: Element, parent: Node, anchor: Node|Node[];
        let counter = 0, val: number, child: RefType;
        for (let element of this.elements) {
            counter++;
            children = this.items.get(element);
            sources = {};
            for (val of Object.keys(what) as any) {
            ...
            for ([i1, i2] of Object.entries(what) as any) {
                if (typeof i1 === 'string') i1 = parseInt(i1);
                c1 = sources[i1];
                ...
                if (!swap) {
                    ...
                    if (counter === 1) this.refs.splice(i2, 0, ...this.refs.splice(i1, 1));
                    children.splice(i2, 0, ...children.splice(i1, 1));

                } else {
                    if (counter === 1) this.refs[i2] = refs[i1];
                    children[i2] = c1;

                }
                if (this.addIndex) c1.set({index: i2});
            }
        }
        return this;
    }
