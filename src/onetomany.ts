 ...
 * @returns
 */
export function one(many: any[], recursive?: boolean, context?: any[], mainItem?: number) {
    const newOne = new One(many, recursive, context);
    newOne.ctor = one; 
    if (mainItem !== undefined) newOne.mainItem = mainItem;
 ...
 * @returns
 */
export function wrap(o: One, mainItem?: number) {
    o.ctor = one;
    if (mainItem !== undefined) o.mainItem = mainItem
    ...
    get(target: One, p: typeof PURE) {
        ...
        if (result.length && typeof result[0] === "function") {
            if (target.mainItem !== undefined) {
                return (...args: any[]) => target.call([args], p)[target.mainItem];
            } else {
            ...
            if (target.ctor) {
                ...
                ((newResult[PURE] || newResult) as One).ctor = target.ctor;
                return newResult;
            }
            else return new One(result, true, target.context);
        }
        if (target.mainItem !== undefined) return result[target.mainItem];
        else return result;
    },
...
export class One {
    ...
    get(
        ...
        if (this.recursive && !forceArray) {
            ...
            } else return new One(results, this.recursive, this.context)
        } else return results;
    }
    ...
    call(
        ...
        if (args === undefined || !args.length) args = [[]];
        const results: unknown[] = [];
        const length = this.many.length;
        const j = args.length;
