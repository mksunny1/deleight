 ...
 */
export async function loadStyle(href: string, init?: RequestInit) {
 ...
 */
export function *popStyles(...elements: Element[]) {
    for (let element of elements) {
        if ((element.getAttribute("rel") === "stylesheet" && element.hasAttribute('href')) || element instanceof HTMLStyleElement) {
            if (element.hasAttribute(popAttrs.skip)) continue;
            if (element instanceof HTMLStyleElement) {
                yield createStyle(element.textContent || '');
            } else {
                yield loadStyle(element.getAttribute("href") as string);
            }
            if (!element.hasAttribute(popAttrs.keep)) element.remove();
        } else {
...
function* selectBase(selectors: string, styleSheet: CSSStyleSheet, firstOnly?: boolean): Iterable<CSSRule> {
 ...
 */
export function* selectAll(selectors: string, styleSheet: CSSStyleSheet): Iterable<CSSRule> {
 ...
 */
export function selectFirst(selectors: string, styleSheet: CSSStyleSheet): CSSRule | undefined {
    for (let rule of selectBase(selectors, styleSheet, true)) return rule;
}
...
export class StyleSheet {
     ...
     */
    add<T extends Element | DocumentFragment | ShadowRoot | Document>(...elements: T[]) {
        let root: Document | ShadowRoot;
        const allElements: (Element | ShadowRoot | Document)[] = [];
     ...
     */
    remove<T extends Element | DocumentFragment | ShadowRoot | Document>(...elements: T[]) {
     ...
     */
    set(p: string, value: string | CSSRule) {
        const rule = selectFirst(p, this.css);
        if (rule) return rule.cssText = (value instanceof CSSRule)? value.cssText: value;
    }
    /**
    ...
    delete(p: string) {
        const rule = selectFirst(p, this.css);
        if (rule) this.css.deleteRule(Array.prototype.indexOf.call(this.css.cssRules, rule));
    }
    /**
     ...
     */
    setAll(p: string, ...values: (string | CSSRule)[]) {
        const rules = selectAll(p, this.css);
        let i = 0, value: string | CSSRule = '';
 ...
 * in its `css` property.
 */
export function wrap(css: CSSStyleSheet) {
